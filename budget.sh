...
if [[ -z "$CATEGORY" ]]; then
  while true; do
    ...
    if [[ "$CATEGORY" =~ ^[Cc][Oo][Ss][Tt]$ || "$CATEGORY" =~ ^[Uu][Ss][Aa][Gg][Ee]$ ]]; then break; fi
    echo "Invalid category. Enter 'Cost' or 'Usage'."
  done
fi

# Check if user selected Usage and fall back to Cost
if [[ "$CATEGORY" =~ ^[Uu][Ss][Aa][Gg][Ee]$ ]]; then
  echo
  echo "[NOTICE] Usage budget category is not currently supported. That functionality will be added soon!"
  echo "[NOTICE] Automatically defaulting category to 'Cost'."
  CATEGORY="Cost"
fi
